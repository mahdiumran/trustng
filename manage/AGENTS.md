...
├── setssh.new             # (removed) — no longer managed from panel
...
## 15. Arsitektur DNS Patched (2026 Migration)

> Bagian ini mendokumentasikan migrasi ke Unbound patched + blocklist otomatis.
> Rollback: `tar xzf /root/opencode/backup-unbound/pre-migration-snapshot.tgz -C /`
> lalu hapus `/etc/systemd/system/unbound.service.d/override.conf` + `daemon-reload`.

### 15.1 Komponen

| Komponen | Path | Fungsi |
|---|---|---|
| unbound patched 1.26.0 | `/usr/local/sbin/unbound{,-checkconf,-control}` | Filter CDB, zona blacklist/whitelist, counter `num.blacklist`, hot-remap |
| Patch source | `/root/opencode/trustng-deploy/unbound-filterdb.patch` | Terhadap unbound 1.26.0 upstream; file baru: `services/filterdb.{c,h}` |
| Blocklist CDB | `/etc/unbound/db/blacklist.db` | ~9.5jt domain (Komdigi), wire format, hot-swappable via rename |
| trust.txt / trust.count | `/etc/unbound/db/` | Dibaca `dbtrust.php` & dashboard (kompatibel lama) |
| Updater | `/usr/local/sbin/update-blocklist` | fetch Komdigi → validasi → build → atomic rename (tanpa reload) |
| Timer update | `update-blocklist.timer` | 02:00 & 14:00 (+random 30m) |
| Metrics arsip | `/var/lib/trustng-metrics/metrics.db` | Sampler tiap 60s (`trustng-metrics.timer`) |
| Auth panel | `/var/lib/trustng-auth/auth.db` | SQLite: users (Argon2id), login_attempts (lockout) |
| resetpass CLI | `/usr/local/sbin/resetpass.sh` | `--generate` / `--password 'X'` / interaktif |
| nftables | `/etc/nftables.conf` | silent-drop :53 utk non-klien; set dari `/etc/client_set{,6}` |

### 15.2 Perilaku blocking

1. Query masuk → worker cek whitelist zone (transparent) → jika cocok: **lolos** (menang atas CDB)
2. `fdb_check()` parent-walk di CDB → HIT: jawab dari apex `blacklist.` dengan qname asli
   (A = IP lamanlabuh dari `lamanlabuh.conf`; HINFO/TXT = dasar hukum)
3. MISS → lanjut local-zones & rekursi normal
4. Setiap HIT menaikkan `total.num.blacklist` (tampil di stats & dashboard)

### 15.3 Halaman panel baru

| Halaman | Fungsi |
|---|---|
| `setblack.php` | Editor blacklist manual → `blacklist.local.db` → trigger updater (sudo) |
| `setwhite.php` | Whitelist editor (menu sidebar Konfigurasi; native local-zone transparent) |
| `resetstats.php` | Reset counter runtime (`unbound-control flush_stats`) |
| `login.php` / `logout.php` | Login page modern (session cookie SameSite=Strict) |
| `setpwd.php` | Account Management (info akun + ganti password) |
| `digtest.php` + `digtest_data.php` | DNS Inspector: AJAX JSON per-domain status resolved/blocked/noanswer; dig WAJIB eksplisit `@127.0.0.1` (resolv.conf box ini di-overwrite Tailscale → default resolver bukan unbound lokal!) |

Menu sidebar: **Blacklist** (grup Konfigurasi), **Reset Stats** (grup Sistem).

### 15.4 Auth (menggantikan Basic Auth nginx)

- `includes/auth.php` — helper PDO SQLite + lockout (5 gagal/15 mnt per IP)
- `includes/auth_guard.php` — auto_prepend_file via nginx `PHP_VALUE`;
  melindungi semua `.php` tanpa edit per-file; exempt: `login.php`, `logout.php`
- Ganti password = naikkan `pw_version` → semua sesi lama hangus otomatis
- `setpwd.php` ditulis ulang (SQLite + sinkron `chpasswd` sistem)
- First boot: flag `setup.mulai` → redirect login.php?setup=1 (atur password awal)
- CSRF token pada form login/setpwd; form lama tetap referer-guard

### 15.5 Gotchas baru

18. **Jangan tulis `blacklist.db` in-place** — builder harus ke `.new` lalu `mv`
    (atomic rename); inode berubah agar hot-remap terpicu
19. `unbound-control` tanpa `-c` membaca `/usr/local/etc/unbound/unbound.conf`
    (symlink ke `/etc/unbound/unbound.conf` sudah dibuat — jangan dihapus)
20. Socket control `/etc/unbound/run/unbound.sock` perms 0660+group;
    www-data harus member group `unbound` (sudah di-set usermod + tmpfiles.d)
21. `stat-interval: 0` TETAP menjalankan timer 1 detik untuk remap check
    (patch worker.c) — jangan hapus blok timer
22. Client `unbound-control` versi kita selalu pakai TLS jika config client
    menyertakan cert files; unix socket + `control-use-cert: no` = plaintext UBCT1
23. Panel memanggil `dn` atau `unbound-control` tanpa path — wrapper di
    `/usr/bin/{s,dn}` dan symlink default config menjaga kompatibilitas

*Last updated: 2026 · TRUST-NG DNS Services · Kominfo*
