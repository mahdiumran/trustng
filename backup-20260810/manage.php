...
// --- DNS Statistics ---
// Total queries and blacklist rate from Unbound stats (/usr/bin/s)
$unbound_stats = @shell_exec("/usr/bin/s 2>/dev/null");
$totalQueries = 0;
$blockedQueries = 0;
if ($unbound_stats !== null) {
    // /usr/bin/s outputs: "queries  :   123 queries/s" and "blacklist:     3 queries/s"
    if (preg_match('/queries\s*:\s*(\d+)/i', $unbound_stats, $m)) {
        $totalQueries = intval($m[1]);
    }
    if (preg_match('/blacklist\s*:\s*(\d+)/i', $unbound_stats, $m2)) {
...
<section class="dashboard-main">
  ...
  <div class="tng-topbar">
    ...
    <button id="tng-menu-toggle" title="Toggle Sidebar" aria-label="Toggle menu">
  ...
  <div class="tng-content">
    ...
    <div>
      ...
      <div class="tng-status-strip">
        ...
        <div class="tng-status-card">
          <div class="tng-status-icon">DB</div>
          <div class="tng-status-info">
            <span class="tng-status-name">Trust+ DB</span>
            <span class="tng-status-val"><?php echo htmlspecialchars($trustCountFmt, ENT_QUOTES, 'UTF-8'); ?> entries</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      <p class="tng-section-label">Resource Sistem</p>
      <div class="tng-resource-grid">
        ...
        ?>
        <div class="tng-res-card <?php echo $state; ?>"
             data-resource="<?php echo $label; ?>"
             data-value="<?php echo $value; ?>"
             data-unit="<?php echo $unit; ?>">
          <div class="tng-res-header">
            <i class="fa-solid <?php echo $icon; ?>" aria-hidden="true"></i>
            <span><?php echo $title; ?></span>
          </div>
          <div class="tng-res-value">
            <?php echo $displayValue; ?><span><?php echo $unit; ?></span>
          </div>
          <div class="tng-res-bar">
            <div class="tng-res-bar-fill" style="width:<?php echo $value; ?>%"></div>
          </div>
          <svg class="tng-res-svg zabbix-chart" viewBox="0 0 320 80" preserveAspectRatio="none" aria-hidden="true">
    ...
    <!-- ======================================================
         ...
         ====================================================== -->
    <div>
      <p class="tng-section-label">DNS Queries</p>
    ...
    <!-- ======================================================
         BLOCKLIST QUERY CHART
         ====================================================== -->
    <div>
      <p class="tng-section-label">Blocklist Queries</p>
      <div class="tng-query-card">
        <div class="tng-query-head">
          <div class="tng-query-meta">
